    ...
    @foreach ($blog_footer as $blog_footer_item)
        <div class="recent-post-item active pb-3">
            <div class="recent-post-image ml-3"><a href="#"><img width="80" height="80"
                        src="{{ asset($blog_footer_item->image) }}" alt="{{ $blog_footer_item->name }}"></a></div>
            <div class="recent-post-text">
                <h6><a href="{{ route('articles.single', $blog_footer_item->id) }}">{{ $blog_footer_item->name }} </a>
                </h6>
                <span class="rcomment">{{ $blog_footer_item->date_fa }} </span>
            </div>
        </div>
    @endforeach
@endsection
