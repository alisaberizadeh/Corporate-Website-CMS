            ...
            <form action="{{ route('social.update') }}" method="POST" enctype="multipart/form-data"> @csrf @method('put')
                <div class="card card-primary mt-3">
                    <div class="card-header">
                        <h3 class="card-title"> بخش بندی </h3>
                    </div>
                            ...
                            @foreach ($sections as $sections_item)
                                <li
                                    ...
                                    @switch($sections_item->status)
                                        @case(0)
                                    <a href="{{route('sections.show',$sections_item->id)}}" class="btn btn-sm btn-success text-white">نمایش بده</a>
                                            
                                            @break
                                        @case(1)
                                    <a href="{{route('sections.hide',$sections_item->id)}}" class="btn btn-sm btn-danger text-white">مخفی کن</a>
